/**
 * 廣清宮記帳軟體 - Supabase雲端同步服務 v2.0
 * 提供真正的雲端數據庫同步功能
 * 全新版本 - 確保沒有快取問題
 */

// Supabase配置 - 支援環境變數和多重來源
const SUPABASE_CONFIG_V2 = {
    url: (() => {
        ...
        try {
            if (typeof window.supabase !== 'undefined') {
                this.supabase = window.supabase.createClient(
                    SUPABASE_CONFIG_V2.url,
                    SUPABASE_CONFIG_V2.key
                );
                console.log('Supabase客戶端初始化成功 v2');
    ...
    async testConnection() {
        try {
            const { data, error } = await this.supabase
                .from(SUPABASE_CONFIG_V2.tableName)
                .select('count')
                .limit(1);
...
// 創建全局實例 v2
window.supabaseSyncV2 = new SupabaseSyncServiceV2();

// 同時設定為原有的變數名以保持兼容性
window.supabaseSync = window.supabaseSyncV2;

console.log('Supabase 同步服務 v2.0 已載入');
